...
df.rename(columns={'duration_ms': 'duration', 'time_signature': 'tsign',
          'popularity': 'song_popularity'}, inplace=True)
df.head()
...
for i in range(len(nf.columns)):
    plt.subplot(5, 2, i+1)
    sns.histplot(nf[nf.columns[i]], color=list(
        np.random.randint([255, 255, 255])/255))
plt.show()
...
for i in range(len(nf.columns)):
    plt.subplot(5, 2, i+1)
    sns.boxplot(nf.columns[i], data=nf, color=list(
        np.random.randint([255, 255, 255])/255))
plt.show()
...
# Removing outliers
df1 = df.copy()
features1 = nf[["acousticness", "danceability", "duration", "energy",
...
print('Before removal of outliers, The dataset had {} samples.'.format(
    df.shape[0]))
...
for i in range(len(nf1.columns)):
    plt.subplot(5, 2, i+1)
    sns.histplot(nf1[nf1.columns[i]], color=list(
        np.random.randint([255, 255, 255])/255))
plt.show()
...
for i in range(len(nf1.columns)):
    plt.subplot(5, 2, i+1)
    sns.boxplot(nf1.columns[i], data=nf1, color=list(
        np.random.randint([255, 255, 255])/255))
plt.show()
...
print(df['popularity'].value_counts())
x_popularity = df[['key', 'mode', 'tsign', 'duration', 'acousticness', 'danceability',
                   'energy', 'liveness', 'loudness', 'speechiness', 'tempo', 'valence']]
y_popularity = df['popularity']
X_train, X_test, y_train, y_test = train_test_split(
    x_popularity, y_popularity, test_size=0.2, stratify=y_popularity, random_state=1)
...
print("Accuracy: {:.5f}%".format(
    knnmodel.score(x_popularity, y_popularity)*100))
...
print("Accuracy: {:.5f}%".format(
    mlpmodel.score(x_popularity, y_popularity)*100))
...
print("Accuracy: {:.5f}%".format(
    rfcmodel.score(x_popularity, y_popularity)*100))
